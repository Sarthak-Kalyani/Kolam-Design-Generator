...
def reset_canvas(rows, cols, spacing):
    pen.clear()
    w = int((cols - 1) * spacing + 400)
    h = int((rows - 1) * spacing + 400)
...
def pattern_multi_layer_diamond(rows, cols, spacing, ox, oy, color="white"):
    pen.color(color)
...
def pattern_concentric_loops(rows, cols, spacing, ox, oy, color="white"):
    ...
    maxr = min(cols, rows) * spacing / 1.2
    for i in range(1, 6):
        r = maxr * (i / 6)
        pen.penup(); pen.goto(midx, midy - r); pen.pendown()
        pen.circle(r)

# ---------------- Free-form Kolams (big, centered) ----------------
...
def pattern_lotus_center(rows, cols, spacing, ox, oy, color="white"):
    pen.color(color)
    pen.penup(); pen.goto(0, -120); pen.pendown()
    for i in range(36):
        pen.circle(120, steps=6)
        pen.right(170)
...
def preset_5x5():
    ...
    spacing_entry.delete(0, tk.END); spacing_entry.insert(0, "70")
